...
function Nav({ page, setPage }) {
  ...
  return (
    <nav style={{ background: '#f2ebe3', borderBottom: '1px solid #ede5dc', position: 'relative', zIndex: 50 }}>
      <div style={{ padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
        <div>
          <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 18, fontWeight: 400, fontStyle: 'italic', color: '#1a1714', margin: 0 }}>Paige Camryn Music</p>
          <p style={{ fontSize: 8, letterSpacing: '.16em', textTransform: 'uppercase', color: '#9a9189', margin: 0 }}>Luxury Event Harpist</p>
        </div>

        {/* Desktop nav */}
        <div style={{ display: 'flex' }} className="public-nav-desktop">
          {pages.map(p => (
        ...
        <button
          ...
          style={{
            background: '#c9a097', border: 'none', borderRadius: 8,
            padding: '10px 16px', cursor: 'pointer',
            fontFamily: 'Jost, sans-serif', fontSize: 12,
            color: 'white', fontWeight: 600, letterSpacing: '.06em'
          }}
        >
          {menuOpen ? '✕' : '☰ Menu'}
        </button>
      ...
      {menuOpen && (
        <div className="public-nav-dropdown" style={{ background: '#f2ebe3', borderTop: '1px solid #ede5dc' }}>
          {pages.map(p => (
            <button key={p} onClick={() => { setPage(p); setMenuOpen(false) }} style={{
              display: 'block', width: '100%', padding: '16px 24px',
              border: 'none', borderBottom: '1px solid #ede5dc',
              background: page === p ? '#f5e6e2' : 'transparent',
              fontFamily: 'Jost, sans-serif', fontSize: 15, fontWeight: page === p ? 600 : 400,
